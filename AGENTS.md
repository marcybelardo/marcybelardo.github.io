...
`pnpm test` runs serial missing-image-alt validation for project and blog images, verifies temporary draft routes stay out of production, builds optional-field project fixtures while excluding the draft fixture, runs the clean production build, and executes the dependency-free Node test suite. The generated-output tests cover project index/detail routes, optional square-wrapper ancestry, no-JavaScript navigation, and relationship filtering; `tests/navigation-runtime.test.ts` executes the built navigation script against a browser-event-order DOM harness for open/close, deferred outside-click focus restoration, breakpoint ARIA synchronization, and complete swap listener cleanup. `pnpm verify` aliases `pnpm test`.
