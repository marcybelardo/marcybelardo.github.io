...
`pnpm test` runs serial missing-image-alt and invalid-project-URL validation, verifies temporary draft routes stay out of production, builds full optional-field and no-optionals project fixtures while excluding the draft fixture, runs the clean production build, and executes the dependency-free Node 22 built-in test suite with TypeScript stripping. The generated-output tests cover project index/detail routes, tag-free visible JSON-LD, no-cover index measure variants, prose-only `.md`/`.mdx` bodies across Markdown/HTML/JSX image forms, intrinsic responsive square-image contracts for cover and gallery images, no-optionals section omission, no-JavaScript navigation, relationship filtering, sitemap/robots/404 discovery, and migration contracts; `tests/margin-note-layout.test.ts` covers ordered collision-free offsets, `tests/margin-notes.test.ts` covers injected-port restoration and cleanup failures, and `tests/margin-notes-browser.test.ts` exercises `initializeMarginNotes()` against before/after-print events, media-query changes, resize scheduling, incomplete coverage, and restoration failures; `tests/navigation-runtime.test.ts` executes the built navigation script against a browser-event-order DOM harness for open/close, deferred outside-click focus restoration, breakpoint ARIA synchronization, and complete swap listener cleanup. `pnpm verify` aliases `pnpm test`.
