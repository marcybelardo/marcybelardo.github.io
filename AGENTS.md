...
Astro Markdown keeps built-in GFM footnotes and applies the local rehype margin-note annotation plugin only to paired reference/definition nodes, preserving semantic links, backlinks, and one definition copy.
Blog detail pages keep the generated footnote section as the baseline and progressively enhance it with the injected margin-note controller only after wide-layout measurements succeed; print, narrow layouts, JavaScript-disabled pages, and placement failures restore readable endnotes.
...
`pnpm test` runs serial missing-image-alt and invalid-project-URL validation, verifies temporary draft routes stay out of production, builds full optional-field and no-optionals project fixtures while excluding the draft fixture, runs the clean production build, and executes the dependency-free Node test suite. The generated-output tests cover project index/detail routes, tag-free visible JSON-LD, no-cover index measure variants, prose-only `.md`/`.mdx` bodies across Markdown/HTML/JSX image forms, intrinsic responsive square-image contracts for cover and gallery images, no-optionals section omission, no-JavaScript navigation, and relationship filtering; `tests/margin-note-layout.test.ts` covers ordered collision-free offsets, while `tests/margin-notes.test.ts` covers injected-port restoration, placement failures, and before-print backlink preservation; `tests/navigation-runtime.test.ts` executes the built navigation script against a browser-event-order DOM harness for open/close, deferred outside-click focus restoration, breakpoint ARIA synchronization, and complete swap listener cleanup. `pnpm verify` aliases `pnpm test`.
