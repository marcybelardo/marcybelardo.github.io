...
- Standard.site is manual-only: Markdown/frontmatter and the deployed site remain authoritative, while `src/standard-site/registry.json` stores only public opaque publication/document AT-URIs. No Astro build, test, CI job, or deployment may perform PDS writes, network sync, or read secrets. Use the manual create/update/delete procedure in `docs/standard-site-manual.md`; preserve each mapped URI, use explicit stable blog slugs, and perform live `.well-known`/document-link verification after deployment.
