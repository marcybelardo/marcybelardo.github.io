...
- `BaseLayout` canonical overrides are reserved for same-origin migration destinations and are validated against `SITE_ORIGIN`. `SquareImage` requires meaningful alt text by default; an empty alt is valid only with an explicit `decorative={true}` prop.
