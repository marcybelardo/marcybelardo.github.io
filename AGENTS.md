...
This branch (`codex/photo-wide`) is a local design study for comparison with the other photo-layout branch. The homepage uses `src/styles/home-design.css` and sentence-case shared utility typography. It now consists only of the name, photograph and page links; the reading widths, routes, and blog behavior remain intact. `PhotoStudy.astro` reserves the photograph space and accepts `src/assets/marcy_sensouji.jpg` with its meaningful alt. The homepage photo uses edge-to-edge, viewport-relative height and centered cover cropping; project `SquareImage` behavior remains unchanged while Bio uses its native 2:3 portrait triptych. The homepage uses the existing focus-managed menu at all widths through `data-compact="always"`, with visible in-flow links when JavaScript is disabled. See `docs/homepage-design-comparison.md` for preview and photo instructions.

### Photo-wide landing and Bio refinement

`home-design.css` sizes the title/photo/directions composition to 100svh, with minimum height allowing overflow on short screens or enlarged text rather than clipping content. The homepage footer is hidden. `photo-navigation.css` supplies a blank Prussian-blue square menu control on Home and Bio, preserving the accessible name, focus behavior, and visible no-JavaScript baseline. `bio-design.css` arranges the supplied mirror photograph as a native 2:3 grayscale triptych with compact marginal text and readable desktop geometry; it stacks on small screens. The first image uses the descriptive alt `Marceline Belardo taking a mirror photograph with a camera`; the two repeated images are decorative with empty alt text. Bio copy remains in `bioContent`. The homepage photo alone uses centered cover cropping.
