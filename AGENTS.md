...
This branch (`codex/photo-wide`) is a local design study for comparison with the other photo-layout branch. The homepage uses `src/styles/home-design.css` and sentence-case shared utility typography. It now consists only of the name, photograph and page links; the reading widths, routes, and blog behavior remain intact. `PhotoStudy.astro` reserves the photograph space and accepts `src/assets/homepage-photo.jpg`; set its meaningful `alt` before building with a photo. The homepage photo uses edge-to-edge, viewport-relative height and centered cover cropping; project and Bio SquareImage behavior is unchanged. The homepage uses the existing focus-managed menu at all widths through `data-compact="always"`, with visible in-flow links when JavaScript is disabled. See `docs/homepage-design-comparison.md` for preview and photo instructions.

### Photo-wide landing and Bio refinement

`home-design.css` sizes the title/photo/directions composition to 100svh, with minimum height allowing overflow on short screens or enlarged text rather than clipping content. The homepage footer is hidden. `photo-navigation.css` supplies a blank Prussian-blue square menu control on Home and Bio, preserving the accessible name, focus behavior, and visible no-JavaScript baseline. `bio-design.css` arranges the existing SquareImage portrait with compact marginal text and larger gaps; it stacks on small screens. Bio copy remains in `bioContent`. The homepage photo alone uses centered cover cropping.
