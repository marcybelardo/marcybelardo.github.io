...
The shared `BaseLayout` owns the compact dark-square, focus-managed primary navigation and the site-wide GA4 (`G-4EKN42R6PS`) and Google Tag Manager (`GTM-N4JNKN2H`) integrations. Its only primary destinations are Projects, Blog, and About. Code, Paintings, and Photography are retired and must not be generated or added to the sitemap. The panel is server-rendered open for a JavaScript-disabled baseline; the browser script adds `data-js-ready` and progressively enhances it into a closed, focus-managed menu on every route. A small home link appears on every page except the homepage. `GlowText.astro` owns the paired decorative and semantic text copies, while `BaseLayout` loads and initializes ink-hover once.
Projects and About are working static destinations. Projects reads published entries from the project collection. About contains verified profile copy, the CV link, the native 2:3 grayscale mirror-photo triptych with one descriptive alt and two decorative repetitions, quiet skills text, an inline email link, and the shared social icon footer. `/bio/` and `/contact/` are lightweight noindex static redirects to `/about/`.
The Projects index is a full-width typographic catalogue backed by `ProjectIndexEntry`, with ordinal/year, title and description, and a right-side metadata rail; titles use `GlowText`, and covers occupy a dedicated column only when present. Published project IDs generate static case-study routes under `/projects/<slug>/` through `ProjectLayout`, which pairs an expressive `GlowText` title and description with marginal metadata/links, a readable body column, and spacious optional media. Project covers and galleries use `SquareImage`, and related project/writing links are resolved against published entries while preserving frontmatter order. Project-specific responsive styling lives in `src/styles/projects-design.css`; the layout keeps current sparse entries deliberate without adding copy or imagery. See `docs/projects-design.md` for the composition contract.
On the photo-wide branch, the homepage is a viewport-height photographic landing with one title, a smaller right-aligned photograph, and direct Projects, Blog, and About links. It intentionally has no practice statement or project/writing previews; the About page holds the verified work and practice summary, skills, CV link, portrait, and direct contact line. Shared featured/recent queries remain available to other consumers.
...
The canonical origin is `https://www.marcelinebelardo.com`; use it for canonical, RSS, and sitemap URLs. The static route tree is `/`, `/projects/`, `/projects/<slug>/`, `/blog/`, `/blog/<slug>/`, `/blog/tags/<tag>/`, `/about/`, `/bio/` (redirect), `/contact/` (redirect), `/rss.xml`, `/sitemap-index.xml`, `/sitemap-0.xml`, `/robots.txt`, and `/404.html`. `/code/`, `/paintings/`, and `/photography/` are intentionally absent.
...
- Blog posts live in `src/content/blog/` and should set an explicit stable lowercase `slug`, which produces `/blog/<slug>/`. Title and ISO date are required; published posts require a non-empty description. Optional image fields are paired: `image` requires meaningful `imageAlt`. Standard GFM footnotes use `[^id]` and `[^id]: definition`. Blog index and tag routes use the shared editorial base plus `src/styles/blog-design.css`; detail routes use the editorial body and margin-note styles with the same dedicated blog stylesheet. Do not add utility-class styling or a `.prose` dependency. The title-derived ID is compatibility fallback only.
...
- Drafts are excluded from production detail routes, tag archives, homepage sections, relationships, RSS, and sitemap output. Run `pnpm verify` after content or About edits.
- About authoring edits the `aboutContent` block in `src/pages/about/index.astro`. Keep profile copy, alt text, education, email, and CV details factual and verified. `/bio/` and `/contact/` are noindex compatibility redirects to `/about/` and must stay out of the sitemap.
...
This branch (`codex/photo-wide`) continues the photo-wide composition and consolidates profile and contact information into About. The homepage uses `src/styles/home-design.css` and the shared compressed display face. It consists only of the name, a smaller right-aligned photograph, and Projects, Blog, and About links; the reading widths, routes, and blog behavior remain intact. `PhotoStudy.astro` reserves a constrained photograph frame and accepts `src/assets/marcy_sensouji.jpg` with its meaningful alt. The homepage photo uses a right-aligned width of `min(72vw, 64rem)` on wide screens and full width on mobile, with centered cover cropping; project `SquareImage` behavior remains unchanged while About uses its native 2:3 portrait triptych. The shared focus-managed compact menu appears across Home, About, Projects, Blog, project details, blog details, and tag archives through `data-compact="always"`, with visible in-flow links when JavaScript is disabled. The homepage hides the small home brand, expands its header to the landing-page edge gutter, and pins the enhanced menu to the far right while reserving title space. Home and About text use `GlowText.astro`; `BaseLayout` provides the progressive `ink-hover` pointer field to text targets and the shared menu on fine-pointer devices. See `docs/homepage-design-comparison.md` for preview and photo instructions.

### Photo-wide landing and About composition

`home-design.css` sizes the title/photo/directions composition to 100svh, with minimum height allowing overflow on short screens or enlarged text rather than clipping content. The homepage footer is hidden. `compact-navigation.css` supplies the shared dark-square menu control and small home brand, preserving the accessible name, focus behavior, and visible no-JavaScript baseline. On Home, the header uses the landing-page edge gutter, the enhanced square sits at far right, and its panel opens below/right of the reserved title area. The shared header lets the page beneath receive pointer events while keeping the menu controls interactive. `ink-hover.css` keeps real text fully opaque black and layers a grainy, multicolor duplicate beneath it in an `aria-hidden` decorative span; a pointer-following radial mask and 8px blur reveal a localized glow that bleeds about 6–12px around letter edges. `GlowText.astro` renders the paired copies and `BaseLayout` initializes the pointer field once. The menu aura uses an oversized, blurred textured multicolor field behind its black foreground square, without a surface mask. Fine-pointer setup remains progressive; touch, coarse-pointer, and no-JavaScript states stay black. `interior-page.css` defines starter page-width, heading, and section tokens and classes. `blog-design.css` owns the Blog refinement. `about-design.css` arranges the supplied mirror photograph as a native 2:3 grayscale triptych with compact marginal text, quiet skills text, a full-width email line, and readable desktop geometry; it stacks on small screens. The first image uses the descriptive alt `Marceline Belardo taking a mirror photograph with a camera`; the two repeated images are decorative with empty alt text. About copy remains in `aboutContent`. The homepage photo alone uses centered cover cropping.

### Blog composition

The Blog index and tag archives share `BlogIndexEntry`, which owns the post date, glowing linked title, description, optional square cover, and tag links so both routes remain consistent. `blog-design.css` makes these pages a full-width editorial contents list with the RSS link opposite the heading. Blog detail pages use a spacious GlowText title, date and tag metadata, an optional separated image, and the Source Sans 3 reading body. Keep the generated footnote baseline, author signature, RSS content, and all margin-note enhancement, narrow-screen, and print behavior intact. About keeps its shared icon footer and pairs the full-width contact line with the descriptive accessible email label and a localized `GlowText` effect. The legacy Contact route redirects to About.
